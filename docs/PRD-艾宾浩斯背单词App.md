...
### 3.1 每日学习闭环（P0，2026-08-28 升级）

```
打开 App → 今日概览
  → 复习队列：FSRS 到期词（硬约束）+ B类提前巩固词（概率抽取 ≤5）
  → 新词学习卡（可自评"不熟悉"直接进薄弱词）
  → 【当日新词验收测试】4选1：答对→B类（明天必复习）/ 答错→C类（薄弱词+短间隔）
  → 今日完成 → 自动为当天薄弱词定制个性化例句（每词×随机1~2个其他薄弱词）
```
...
| 类 | 判定 | 调度策略 |
|---|---|---|
| A 熟悉词 | 定级认识 / FSRS 稳定性 ≥180 | 不再出现 |
| B 刚学会 | 学习+验收测试通过 | D+1 必复习（FSRS 学习步）；之后未到期时按 P=(1−R)×0.5 概率提前巩固（≤5个/天） |
| C 没学会 | 验收答错/自评不熟悉/复习忘记 | FSRS 短间隔（当天重学+次日）+ 必进薄弱词个性化 |

规则：

- 到期复习 > 当日上限时，超出部分顺延次日（积压合并），UI 明确告知。
- 新词额度 = min(10, 上限 − 当日复习量 − 提前巩固量)；复习优先。
- 断签恢复：逾期词并入次日队列，受上限截断，不做惩罚性堆叠。
- **个性化例句只为薄弱词服务**（2026-08-28 定稿）：新词有自己的三档静态句，不再生成个性化句；已掌握词不再出现在任何复习或个性化场景。
