...
> 最近更新：2026-08-07 · 状态：Web 版功能完整，词库内容生产中（收尾批进行中）
...
| 例句 | 29,129 条（llm-% 成品；三档配齐 8,426 词，有例句的词 10,447） |
| 中文翻译 | 27,124 句（93%） |
| 音频缓存 | 34,028 个 mp3（693MB，`data/tts_cache/`） |
...
| 音频缓存 | `~/vocab-api/data/tts_cache/` | 34,028 文件，磁盘持久 |
...
- 音频缓存体积大不入 git 历史：**GitHub Release 资产 `tts_cache.tar.gz`**（约 690MB），解压至 `data/` 即可完整复现
